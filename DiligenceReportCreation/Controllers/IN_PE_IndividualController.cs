...
{
    public class IN_PE_IndividualController : Controller
    {
        private readonly DataBaseContext _context;
        private readonly IConfiguration _config;
        public IN_PE_IndividualController(IConfiguration config, DataBaseContext context)
        {
            _config = config;
            _context = context;
        }
        ...
        [HttpGet]
        public IActionResult IN_PE_Individual_Edit()
        {
            //Country List
        ...
        [HttpPost]
        public IActionResult IN_PE_Individual_Edit(MainModel mainModel, string SaveData, string Submit)
        {
            string recordid = HttpContext.Session.GetString("recordid");
                            ...
                            try
                            {
                                ...
                                if (educationModel1.Count == 0 || educationModel1 == null)
                                {
                                    for (int i = 0; i < mainModel.diligence.educationList.Count; i++)
                                    {
                                        ...
                                        educationModel.Edu_AdditionalInfo = mainModel.diligence.educationList[i].Edu_AdditionalInfo;
                                ...
                                else
                                {
                                    for (int i = 0; i < educationModel1.Count; i++)
                                    {
                                        ...
                                        educationModel1[i].Edu_AdditionalInfo = mainModel.educationModels[i].Edu_AdditionalInfo;
                        ...
                        else
                        {
                            ...
                            switch (mainModel.diligenceInputModel.Country)
                            {
                                ...
                                case "United Kingdom":
                                    countrySpecific.credit_hits = mainModel.csModel.credit_hits;
                                    countrySpecific.driving_hits = mainModel.csModel.driving_hits;
                                    countrySpecific.driving_hits_doc_required = mainModel.csModel.driving_hits_doc_required;
                                    countrySpecific.criminal_record_hits = mainModel.csModel.criminal_record_hits;
                                    countrySpecific.insolvency_hits = mainModel.csModel.insolvency_hits;
        ...
        [HttpGet]
        public IActionResult IN_PE_Individual_page1()
        {
            //Country List
            List<string> CountryList = new List<string>();
            CultureInfo[] CInfoList = CultureInfo.GetCultures(CultureTypes.SpecificCultures);
        ...
        [HttpPost]
        public IActionResult IN_PE_Individual_page1(MainModel mainModel, string SaveData, string Submit)
        {
            string recordid = HttpContext.Session.GetString("recordid");
            if (Submit == "SubmitData")
            {
        ...
        public IActionResult Save_Page1(MainModel diligenceInput)
        {
            ...
            templatePath = _config.GetValue<string>("INPEIndividualtemplatePath");
            if (diligenceInput.summarymodel.Summarytable.ToString().Equals("Yes") && diligenceInput.otherdetails.Executivesummary.ToString().Equals("Yes"))
            {
                templatePath = string.Concat(templatePath, "IN_PE_Individual_SterlingDiligenceReport(XXXXX)_DRAFT1.docx");
            }
            else
            {
                if (diligenceInput.summarymodel.Summarytable.ToString().Equals("No") && diligenceInput.otherdetails.Executivesummary.ToString().Equals("No"))
                {
                    templatePath = string.Concat(templatePath, "IN_PE_Individual_SterlingDiligenceReport(XXXXX)_DRAFT3.docx");
                }
                else
                {
                    if (diligenceInput.summarymodel.Summarytable.ToString().Equals("Yes") && diligenceInput.otherdetails.Executivesummary.ToString().Equals("No"))
                    {
                        templatePath = string.Concat(templatePath, "IN_PE_Individual_SterlingDiligenceReport(XXXXX)_DRAFT4.docx");
                    }
                    if (diligenceInput.summarymodel.Summarytable.ToString().Equals("No") && diligenceInput.otherdetails.Executivesummary.ToString().Equals("Yes"))
                    {
                        templatePath = string.Concat(templatePath, "IN_PE_Individual_SterlingDiligenceReport(XXXXX)_DRAFT2.docx");
                    }
                }
            }
            ...
            for (int i = 0; i < diligenceInput.EmployerModel.Count; i++)
            {
                ...
                try
                {
                    if (diligenceInput.EmployerModel[i].Emp_Location.Equals(""))
                    {
                    ...
                    {
                        empLocation = diligenceInput.EmployerModel[i].Emp_Location;
                ...
                if (i == 0)
                {
                    if (diligenceInput.EmployerModel[i].Emp_Confirmed.ToString().Equals("Not Reported By Subject"))
                    {
                        emp_Comment = string.Concat("•	[Employer1]\n	Not Reported by Subject\n\nWhile not reported on his application, resume or LinkedIn profile, research efforts utilizing the subject’s Social Security number through an automated third-party employment verification service housing numerous personnel records for thousands of companies revealed that the subject is <currently/previously> employed as a [Position1] at [Employer1], presumably in [EmpLocation1], since [EmpStartDate1].");
                    }
                    else
                    {
                        emp_Comment = string.Concat("•	[Employer1]\n	[EmpLocation1]\n	[Position1]\n	[EmpStartDate1] to Present");
                        switch (diligenceInput.EmployerModel[i].Emp_Confirmed.ToString())
                        {
                            case "No":
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        switch (diligenceInput.EmployerModel[i].Emp_Confirmed)
                        {
            ...
            for (int i = 0; i < diligenceInput.EmployerModel.Count; i++)
            {
                TextSelection[] texteemployer = doc.FindAllString(string.Concat("•	",diligenceInput.EmployerModel[i].Emp_Employer), false, false);
                            ...
                            foreach (Paragraph para in section.Paragraphs)
                            {
                                ...
                                for (int k = 0; k < para.ChildObjects.Count; k++)
                                {
                                    ...
                                    if (obj.DocumentObjectType == DocumentObjectType.TextRange)
                                    {
                                        ...
                                        {
                                            textRange = para.AppendText(string.Concat("Efforts to confirm the subject’s employment with ", diligenceInput.EmployerModel[i].Emp_Employer.ToString(), " are currently ongoing, the results of which will be provided under separate cover, if and when received."));
            ...
            bnres = "";
            string[] arrfind = { "In order to preserve the confidentiality of this investigation,", "As confirmed <Investigator to add method of confirmation>,", "As confirmed through an automated third-party verification service utilized by <investigator to insert company name>,", "Efforts to confirm the subject’s employment with" };
            ...
            for (int i = 0; i < diligenceInput.educationModels.Count; i++)
            {
                ...
                if (diligenceInput.educationModels[i].Edu_History.ToString().Equals("Yes"))
                {
                    ...
                    if (i == diligenceInput.educationModels.Count - 1)
                    {
                        try
                        {
                            if (diligenceInput.educationModels[i].Edu_AdditionalInfo.ToString().Equals("No"))
                            {
                                switch (diligenceInput.educationModels[i].Edu_Confirmed.ToString())
                                {
                                    case "Yes":
                                        ...
                                        edu_header = string.Concat(edu_header, diligenceInput.educationModels[i].Edu_Degree.ToString(), " - ", "Confirmed");
                                    ...
                                    case "No":
                                        ...
                                        edu_header = string.Concat(edu_header, diligenceInput.educationModels[i].Edu_Degree.ToString(), " - ", "Unconfirmed");
                                        edu_summcomment = string.Concat(edu_summcomment, edusumcomment1.unconfirmed_comment.ToString(), "\n\n");
                                        break;
                                    case "Result Pending":
                                        edu_comment = string.Concat(edu_comment, "Efforts to independently confirm the subject’s tenure at the same are currently ongoing, the results of which will be provided under separate cover, if and when received.  ", edurescomment.confirmed_comment.ToString(), "\n\n");
                                        edu_header = string.Concat(edu_header, diligenceInput.educationModels[i].Edu_Degree.ToString(), " - ", "Results Pending");
                                    ...
                                    case "Attendance Result pending":
                                        edu_comment = string.Concat(edu_comment, "Efforts to independently verify the same are currently ongoing, the results of which will be provided under separate cover, if and when received.  ", edurescomment.unconfirmed_comment.ToString(), "\n\n");
                                        edu_header = string.Concat(edu_header, diligenceInput.educationModels[i].Edu_Degree.ToString(), " - ", "Results Pending");
                            ...
                            else
                            {
                                ...
                                edu_comment = string.Concat(edu_comment, "While the subject self-reports that he/she received a [Degree], from [School] on [GradDate], having attended the same from [EduStartDate] to [EduEndDate], additional information from the subject, to include a <investigator to add required information>, would be required in order to confirm the details for the same.", "\n\n");
                            }
                        }
                        catch
                        {
                    ...
                    try
                    {
                        if (diligenceInput.educationModels[i].Edu_Major.Equals("")) { }
                        else
                        {
                            edu_comment = edu_comment.Replace("[Degree]", string.Concat("[Degree] in ", diligenceInput.educationModels[i].Edu_Major));
                        }
                    }
                    catch { }
                    try
            ...
            if (diligenceInput.otherdetails.Has_Business_Affiliations.ToString().Equals("Yes"))
            {
                ...
                business_comment = string.Concat("In addition to the above, <and while not believed to be a comprehensive listing of the same>, research of records maintained by the Secretary of State’s Office, well as other sources, identified the subject in connection with the following business entities:", "\n\n\t", "•\t<Investigator to insert bulleted list of results here>\n\t•\t<Investigator to insert bulleted list of results here>\n\t•\t<Investigator to insert bulleted list of results here>");
            }
            else
            {
            ...
            PMCommentModel = string.Concat(PMCommentModel,"[DESCFAMADESC]");
            doc.Replace("PRESSANDMEDIASEARCHDESCRIPTION", PMCommentModel.ToString(), true, true);
            doc.SaveToFile(savePath);
            //Fama
            switch (diligenceInput.otherdetails.Fama.ToString())
            {
                case "Yes - Clear":
            ...
            else
            {
                ...
                {
                    legrechitcommon = string.Concat("\nIt should be noted that one or more individuals known only as “[FirstName] [LastName]” were identified as <party type> in at least <number> civil filings, which were recorded in [Country]  between <date> and <date>, and manual court records retrieval efforts -- if available -- would be required in order to determine whether any of the same pertain to the subject of interest, which can be undertaken upon request if an expanded scope is warranted.\n");
                }
            }
            ...
            if (diligenceInput.diligenceInputModel.Country.ToString().Equals("Australia") || diligenceInput.diligenceInputModel.Country.ToString().Equals("United Kingdom") || diligenceInput.diligenceInputModel.Country.ToString().Equals("Canada") || diligenceInput.diligenceInputModel.Country.ToString().Equals("France"))
            {
                ...
                {
                    uslegcommentmodel = "In addition to the above, as a precaution, bankruptcy, criminal and civil court records were searched covering a period of at least 10 years or so through the United States District and Bankruptcy Courts, which did not identify any records in connection with the subject, personally.\n\nIt is noted that state- and lower-level court records research efforts would be required in connection with the subject in the United States, should additional investigative efforts be warranted.";
                }
            ...
            if (diligenceInput.diligenceInputModel.Country.ToString().Equals("United Kingdom"))
            {
                ...
                try
                {
                    if (diligenceInput.csModel.driving_hits_doc_required.ToString().Equals("Yes"))
                    {
                        ...
                        uk_dhcommentmodel = "";
                        doc.Replace("DRIVINGHISTORYDESCRIPTION", "Additional information must be provided by the subject in order to obtain the subject’s driving history record in the United Kingdom. However, research conducted in the subject’s relevant jurisdictions did not identify him in connection with any traffic or vehicle-related incidents.", true, true);
                    }
                    else {
                        switch (diligenceInput.csModel.driving_hits.ToString())
                        {
            ...
            doc.Replace("[HeaderCountry]", diligenceInput.diligenceInputModel.Country, true, true);
            doc.Replace("[Country]", diligenceInput.diligenceInputModel.Country, true, true);
            doc.SaveToFile(savePath);
            if (diligenceInput.summarymodel.Summarytable.ToString().Equals("No"))
            {
                ...
                TextSelection[] text39 = doc.FindAllString("	•", false, true);
                if (text39 != null)
                {
                    foreach (TextSelection seletion in text39)
                    {
                ...
                TextSelection[] textSelections2 = doc.FindAllString("Business Affiliation", false, true);
                ...
                try
                {
                    if (diligenceInput.diligenceInputModel.MiddleInitial == "")
                    {
                    ...
                    {
                        doc.Replace("[MiddleInitial]", diligenceInput.diligenceInputModel.MiddleInitial, true, true);
                        doc.Replace("Middle_Initial", diligenceInput.diligenceInputModel.MiddleInitial.TrimEnd().ToUpper().ToString(), true, true);
                    }
                }
                catch
                {
            ...
            {
                SummaryResulttableModel summary = new SummaryResulttableModel();
                summary = diligenceInput.summarymodel;
        ...
        public IActionResult Save_Page2(SummaryResulttableModel ST)
        {
            ...
            switch (ST.civil_court_Litigation.ToString())
            {
                case "Clear":
                    ...
                    break;
                case "Records":
                    civilcourtcomment = "LastName was identified as a <party type> in connection with at least <number> civil filings in [Country], filed between <date> and <date>, which are currently <status>";
                    civilcourtcomment = civilcourtcomment.Replace("LastName", last_name);
                    civilcourtResult = "Records";
                    break;
                case "Record":
                    civilcourtcomment = "[Last Name] was identified as a <party type> in connection with a civil filing in [Country], which was recorded in <date>, which is <status>";
            ...
            if (ST.civil_court_Litigation1 == true)
            {
                ...
                civilcourtcomment = string.Concat(civilcourtcomment, "\n\nOne or more individuals known only as “[FirstName] [LastName]” were identified as <Party Type> in at least <number> civil filings in [Country], which were recorded between <date> and <date>, which are <status>");
            }
